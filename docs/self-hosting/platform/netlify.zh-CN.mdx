...
如果想在 Netlify 上部署 MacoChat，可以按照以下步骤进行操作：
...
点击 Fork 按钮，将 MacoChat 仓库 Fork 到你的 GitHub 账号下。
...
<Callout type={'info'}>
  MacoChat 支持的完整环境变量列表请参考 [📘 环境变量](/zh/self-hosting/environment-variables) 部分
</Callout>
...
进入部署详情，你会看到下述界面，这意味着你的 MacoChat 正在部署中，只需等待部署完成即可。
...
如果你的界面中的 Deploy Log 如下所示，意味着你的 MacoChat 部署成功了。
...
此时，你可以点击「Open production deploy」，即可访问你的 MacoChat 站点

<Image
  alt={'访问你的 MacoChat 站点'}
