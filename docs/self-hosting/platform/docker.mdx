...
We provide a [Docker image][docker-release-link] for you to deploy the MacoChat service on your private device.
...
Use the following command to start the MacoChat service with one click:
...
- For the complete list of environment variables supported by MacoChat, please refer to the [Environment Variables](/zh/self-hosting/environment-ariable) section.
