...
Upgrading the Docker deployment version is very simple, you just need to redeploy the latest MacoChat image. Here are the commands required to perform these steps:

<Steps>
### Stop and Remove the Current Running MacoChat Container

Assuming the MacoChat container is named `lobe-chat`, use the following commands to stop and remove the currently running MacoChat container:
...
### Pull the Latest MacoChat Image

Use the following command to pull the latest Docker image for MacoChat:
...
Redeploy the MacoChat container using the newly pulled image:
...
No need to worry, you won't. All of MacoChat's chat records are stored in your local browser. Therefore, when redeploying MacoChat using Docker, your chat records will not be lost.
