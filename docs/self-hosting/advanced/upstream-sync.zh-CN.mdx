...
<Steps>
###  停止并删除当前运行的 MacoChat 容器

假设 MacoChat 容器的名称是 `lobe-chat`，使用以下指令停止并删除当前运行的 MacoChat 容器：
...
使用以下命令拉取 MacoChat 的最新 Docker 镜像：
...
放心，不会的。MacoChat 的聊天记录全部都存储在你的本地浏览器中。因此使用 Docker 重新部署 MacoChat 时，你的聊天记录并不会丢失。
