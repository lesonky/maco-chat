...
MacoChat supports configuring external identity verification services for internal use by enterprises/organizations to centrally manage user authorization. Currently, it supports [Auth0][auth0-client-page]. This article will introduce how to configure the identity verification service.
...
Click on the "Users Management" in the left navigation bar to enter the user management interface, where you can create users for your organization to log in to MacoChat.

![](https://github.com/CloudPassenger/lobe-chat/assets/30863298/3b8127ab-dc4f-4ff9-a4cb-dec3ef0295cc)

### Configure Environment Variables

When deploying MacoChat, you need to configure the following environment variables:

| Environment Variable | Type | Description |
| --- | --- | --- |
| `ENABLE_OAUTH_SSO` | Required | Enable single sign-on (SSO) for MacoChat. Set to `1` to enable single sign-on. |
...
<Callout>
  After successful deployment, users will be able to authenticate and use MacoChat using the users
  configured in Auth0.
</Callout>
...
<Callout type={'warning'}>
  ...
  MacoChat being abused by external users.
</Callout>
<Callout>
