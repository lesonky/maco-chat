# Custom Plugins

## Installing Custom Plugins

If you wish to install a plugin that is not available in the MacoChat plugin store, such as a custom-developed MacoChat plugin, you can click on "Custom Plugins" to install it:

<Video src="https://github.com/lobehub/lobe-chat/assets/28616219/034a328c-8465-4499-8f93-fdcdb03343cd" />

In addition, MacoChat's plugin mechanism is compatible with ChatGPT plugins, so you can easily install corresponding ChatGPT plugins.

If you want to try installing custom plugins on your own, you can use the following links to try:

- `Custom Lobe Plugin` Mock Credit Card: https://lobe-plugin-mock-credit-card.vercel.app/manifest.json
- `ChatGPT Plugin` Access Links: https://www.accesslinks.ai/.well-known/ai-plugin.json

![image](https://github.com/lobehub/lobe-chat/assets/28616219/bb9cd00f-b20c-4d7b-9c60-b921d350e319) ![image](https://github.com/lobehub/lobe-chat/assets/28616219/bdeb678e-6502-4667-86b1-504221ee7ded)

## Developing Custom Plugins

If you wish to develop a MacoChat plugin on your own, feel free to refer to the [Plugin Development Guide](/en/usage/plugins/development) to expand the possibilities of your AI assistant!
