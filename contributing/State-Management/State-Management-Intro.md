...
The directory structure of the previous version of SessionStore for MacoChat, with high complexity, implements a large amount of business logic. However, with the modularization of slices and the fractal architecture, it is easy to find the corresponding modules, making it easy to maintain and iterate on new features.

```bash
MacoChat SessionStore
├── index.ts
├── initialState.ts
├── selectors.ts
├── slices
...
Based on the provided directory structure of MacoChat SessionStore, we can update the previous document and convert the examples to the implementation of MacoChat's SessionStore. The following is a portion of the updated document:

### Best Practices for MacoChat SessionStore Directory Structure

In the MacoChat application, session management is a complex functional module, so we use the Slice pattern to organize the data flow. Below is the directory structure of MacoChat SessionStore, where each directory and file has its specific purpose:
...
In MacoChat, the SessionStore is designed as the core module for managing session state and logic. It consists of multiple Slices, with each Slice managing a relevant portion of state and logic. Below is a simplified example of the SessionStore implementation:
  ...
  persist(
    subscribeWithSelector(
      devtools(createStore, {
        name: 'MacoChat_Session' + (isDev ? '_DEV' : ''),
      }),
    ),
    persistOptions,
  ),
...
Through this layered and modular approach, we can ensure that MacoChat's SessionStore is clear, maintainable, and easy to extend and test.
